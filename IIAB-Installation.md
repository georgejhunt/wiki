...
Here is the complete list of the steps required. Some may already be done.
   ...
   1. On Raspbian, Ubuntu or Debian doing everything from scratch involves a few simple steps: _(Debian [requires Ansible 2.4.x](https://github.com/iiab/iiab/issues/375#issuecomment-334500152) so that multiple runs of "./runansible" can complete, as such running "ansible --version" and "apt purge ansible" is often required to remove Ansible 2.2.x)_
```
         apt update
         apt -y dist-upgrade
         apt -y clean
