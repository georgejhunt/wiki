...
* Anybody can now install Internet-in-a-Box (IIAB) over regular Wi-Fi — or accelerate your install using an Ethernet cable to live Internet.  On Raspberry Pi, use `raspi-config` to connect to your home's Wi-Fi SSID and password prior to installing IIAB.  Then use `iiab-hotspot-on` after IIAB installation is complete!
