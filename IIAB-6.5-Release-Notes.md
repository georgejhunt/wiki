...
* Anybody can now install Internet-in-a-Box (IIAB) over regular Wi-Fi.  On Raspberry Pi, use `raspi-config` to connect to your home's Wi-Fi SSID and password prior to installing IIAB.  Then run `iiab-hotspot-on` after IIAB installation is complete!  Or: accelerate your install if you have an Ethernet cable to live Internet (Ethernet is almost always more reliable than Wi-Fi).
