...
* Installing IIAB over Wi-Fi is complicated by a DNS/dnsmasq failure en route.  Workaround: when screen output freezes or you see DNS errors like "Could not resolve 'raspbian.raspberrypi.org'", you will need to reboot, then re-run `sudo iiab` (sometimes repeatedly!)  Finally, after it auto-reboots (typically within the hour, indicating IIAB software is installed) don't forget to then enable your "knowledge hotspot" by running `iiab-hotspot-on` — as the Admin Console equivalent is not yet working.  Or avoid these issues entirely, by using Ethernet instead of WiFi to install IIAB!  <sub><sub>[#1519](https://github.com/iiab/iiab/issues/1519)</sub></sub> <sub><sub>[#1520](https://github.com/iiab/iiab/issues/1520)</sub></sub>