...
* Installing IIAB over Wi-Fi will fail, and is unnecessarily complicated.  Workaround: when screen output freezes or you see DNS errors like "Could not resolve 'raspbian.raspberrypi.org'", you will need to reboot, then re-run `sudo iiab` (sometimes repeatedly).  After it eventually auto-reboots, indicating IIAB software is installed, run `iiab-hotspot-on` as the Admin Console equivalent is not currently working.  To avoid all these hassles, please use Ethernet instead of WiFi to install IIAB for now!  <sub><sub>[#1519](https://github.com/iiab/iiab/issues/1519)</sub></sub> <sub><sub>[#1520](https://github.com/iiab/iiab/issues/1520)</sub></sub>